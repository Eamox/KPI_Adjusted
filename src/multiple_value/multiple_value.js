...
class MultipleValue extends React.PureComponent {
	...
	var compPoints = dataPoints.slice(1);
    ...
    render() {
	const {config, measure ,target,  data} = this.props;
	let CONFIG = this.props.config;
	let firstPoint = data[0][measure.name];
	let firstTarget = data[0][target.name]|| "";
    ...
    if(config.value_format == "General"){
        if(Math.abs(firstPoint.value) > 1000000) {
	    format = '#,##0.0,,"M"';
        }
        else if (Math.abs(firstPoint.value) > 1000){
            format =  '#,##0.0,"K"';
        }
        else{
            format = "General";
